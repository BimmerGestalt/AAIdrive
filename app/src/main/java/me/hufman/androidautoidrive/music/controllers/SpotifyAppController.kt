...
class SpotifyAppController(context: Context, val remote: SpotifyAppRemote, val webApi: SpotifyWebApi, val appSettings: MutableAppSettings, val isProbing: Boolean) : MusicAppController {
    companion object {
        ...
        fun MusicMetadata.Companion.fromSpotify(track: Track, coverArt: Bitmap? = null): MusicMetadata {
            ...
            return MusicMetadata(
                mediaId = track.uri, queueId = track.uri.hashCode().toLong(),
        ...
        fun MusicMetadata.toListItem(): ListItem {
            return ListItem(
                this.mediaId, this.mediaId, null, this.title, this.subtitle,
    ...
    val CUSTOM_ACTION_TURN_REPEAT_ALL_ON = CustomAction.fromSpotify(
        "TURN_REPEAT_ALL_ON",
        context.getDrawable(R.drawable.spotify_repeat_off)
    )
    val CUSTOM_ACTION_TURN_REPEAT_ONE_ON = CustomAction.fromSpotify(
        "TURN_REPEAT_ONE_ON",
        context.getDrawable(R.drawable.spotify_repeat_on)
    )
    val CUSTOM_ACTION_TURN_REPEAT_ONE_OFF = CustomAction.fromSpotify(
        "TURN_REPEAT_ONE_OFF",
        context.getDrawable(R.drawable.spotify_repeat_one)
    )
    val CUSTOM_ACTION_ADD_TO_COLLECTION = CustomAction.fromSpotify(
        "ADD_TO_COLLECTION",
        context.getDrawable(R.drawable.spotify_add_library)
    )
    val CUSTOM_ACTION_REMOVE_FROM_COLLECTION = CustomAction.fromSpotify(
        "REMOVE_FROM_COLLECTION",
        context.getDrawable(R.drawable.spotify_added_library)
    )
    val CUSTOM_ACTION_START_RADIO = CustomAction.fromSpotify(
        "START_RADIO",
     ...
     */
    private fun loadPaginatedItems(
        parentItem: ListItem,
        stillValid: () -> Boolean,
        onComplete: (List<MusicMetadata>) -> Unit
    ) {
     ...
     */
    private fun loadPaginatedItems(
        destination: MutableList<MusicMetadata>,
        parentItem: ListItem,
        stillValid: () -> Boolean,
        onComplete: (List<MusicMetadata>) -> Unit
    ) {
        remote.contentApi.getChildrenOfItem(parentItem, 200, destination.size).setResultCallback { items ->
            if (items != null && stillValid()) { // this is still a valid request
                destination.addAll(
                    items.items.filterNotNull().map { listItem: ListItem ->
